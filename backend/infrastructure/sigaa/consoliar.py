"""
sigaa_Consolidar.py — Consolida matrícula em atividade complementar / TCC no SIGAA.
...
Fluxo:
  ...
  [6/8]  Na lista de matrículas, localizar a linha do aluno + componente e clicar na seta
  [7/8]  Selecionar Conceito (padrão: E)
  [8/8]  Senha + confirmação final
...
# Mapeia sigla curta → nome EXATO que aparece na lista de consolidação do SIGAA
MAPA_COMPONENTE_CONSOLIDAR = {
...
class EntradaConsolidacao:
    ...
    periodo: str
    polo: str
    componente: str   # sigla curta (ACC I, TCC I, …)
...
def validar_entrada(entrada: EntradaConsolidacao) -> None:
...
async def _clicar_menu_atividades_consolidar(page) -> bool:
    ...
    registra handlers via addEventListener.
    """
    ...
    # ====================================================================
    # ESTRATEGIA PRINCIPAL: hover Atividades → encontrar "Consolidar Matrículas"
    # ====================================================================
    ...
    for tentativa in range(3):
        ...
        # Verificar que caiu na pagina de consolidacao (JSF forward, mesma URL possivel)
        try:
    ...
    # ====================================================================
    # FALLBACK: enumerar itens do submenu Atividades e procurar Consolidar
    # ====================================================================
...
async def _selecionar_discente_componente(page, matricula: str, componente_nome: str) -> bool:
    """
    Na pagina 'Consolidar Matrículas' o SIGAA exibe uma tabela agrupada por
    componente curricular.  Cada linha de aluno tem:
      Matrícula | Discente | (vazio) | Status | Período | seta (→)

    Os componentes aparecem como linhas-titulo (ex: "ATIVIDADES COMPLEMENTARES IV").
    Precisamos encontrar a linha que contém a matrícula E que esteja sob o
    cabeçalho do componente correto.

    A seta de seleção é um <a> ou <input type="image"> com src="seta.gif"
    na mesma <tr> da matrícula.
    """

    await page.wait_for_timeout(1000)

    # Debug: listar estrutura da tabela
    try:
        debug = await page.evaluate("""(params) => {
            ...
            for (const tr of trs) {
                const text = tr.textContent.trim();
                // Detectar linhas de cabeçalho de componente (negrito, sem seta)
                ...
                // Detectar linhas com a matricula
                if (text.includes(matricula)) {
                    ...
                    info.matchingRows.push({
                        ...
                        linkDetails: [...links].slice(0, 5).map(l => ({
                            tag: l.tagName,
                            src: l.getAttribute('src'),
                            onclick: (l.getAttribute('onclick') || '').substring(0, 150),
    ...
    # ── Estratégia 1: JS — encontrar a linha certa por matrícula + componente ──
    # Percorre todas as <tr> mantendo um tracker do componente-cabeçalho atual.
    # Quando acha a matrícula sob o componente correto, retorna o índice da seta.
    clicked = False
    try:
        result = await page.evaluate("""(params) => {
            ...
            for (const tr of trs) {
                ...
                // Linha de cabeçalho de componente: geralmente 1-2 tds, texto todo maiusculo
                // Ex: "ATIVIDADES CURRICULARES COMPLEMENTARES I"
                ...
                if (text.includes(matricula) && currentComponent.includes(compNorm.substring(0, 20))) {
                    ...
                    if (currentComponent === compNorm
                        ...
                        if (seta) {
                            // Se é um <img> dentro de <a>, clicar no <a>
        ...
        if result and result.get("found"):
            ...
            # Clicar via Playwright locator (mais confiável para <input type=image>)
            # Tentar primeiro pelo locator
                ...
                if await seta.count():
                    ...
                    try:
                        ...
                        if _is_navigation_error(click_err) or page.url != url_antes:
                            print(f"   [OK] Click seta navegou. URL={page.url}")
                            return True
                        # Verificar se mudou de conteudo (JSF forward)
                        await page.wait_for_timeout(2000)
                        if page.url != url_antes:
                            return True
                    else:
                        print(f"   [OK] Click seta. URL={page.url}")
                        return True

            # Se o locator nao funcionou, tentar via coordenadas do evaluate
            print(f"   [DEBUG] Tentando click por coordenadas ({result['x']:.0f}, {result['y']:.0f})")
            url_antes = page.url
            try:
                async with page.expect_navigation(timeout=10000):
                ...
                if _is_navigation_error(e) or page.url != url_antes:
                    print(f"   [OK] Click coordenadas navegou. URL={page.url}")
                    return True
                await page.wait_for_timeout(2000)
                if page.url != url_antes:
                    return True
            else:
                print(f"   [OK] Click coordenadas. URL={page.url}")
                return True

    except Exception as e:
        if _is_navigation_error(e):
            return True
        print(f"   [WARN] Estrategia JS falhou: {e}")

    # ── Estratégia 2: se há apenas UMA linha com a matrícula, clicar nela ──
    print("   [FALLBACK] Tentando clicar em qualquer seta na linha da matricula...")
    try:
        ...
        if _is_navigation_error(e):
            return True
        print(f"   [WARN] Fallback falhou: {e}")

    print(f"   [ERRO] Nao foi possivel selecionar matricula {matricula} sob componente {componente_nome}.")
    return False
...
async def executar_consolidacao(args: argparse.Namespace) -> None:
    ...
    componente_nome = MAPA_COMPONENTE_CONSOLIDAR[entrada.componente]
    ...
    async with async_playwright() as p:
        ...
        print("[3/8] Abrindo Portal Coord. Graduacao...")
        ...
        # ── SELEÇÃO DE CURSO ───────────────────────────────────────────────
        print("[4/8] Selecionando curso pelo polo...")
        await page.wait_for_load_state("domcontentloaded")
        ...
        print("[5/8] Menu Atividades > Consolidar Matriculas...")
        if not await _clicar_menu_atividades_consolidar(page):
            try:
                await page.screenshot(path="/tmp/sigaa_consolidar_menu_fail.png")
                print("   [DEBUG] Screenshot salvo em /tmp/sigaa_consolidar_menu_fail.png")
            except Exception:
                pass
            raise RuntimeError("Nao foi possivel navegar Atividades > Consolidar Matriculas.")

        # Aguardar pagina de consolidacao carregar
        try:
        ...
        # Verificar que estamos na pagina de consolidacao
        body_text = await page.locator("body").inner_text()
        if "consolidar" not in body_text.lower() and "lista de matr" not in body_text.lower():
            print(f"   [WARN] Pagina nao parece ser de consolidacao. URL={page.url}")
            print(f"   [DEBUG] Body inicio: {body_text[:300]}")

        # ── SELECIONAR DISCENTE + COMPONENTE NA LISTA ──────────────────────
        print(f"[6/8] Selecionando discente {entrada.matricula} em '{componente_nome}'...")
        if not await _selecionar_discente_componente(page, entrada.matricula, componente_nome):
            try:
                await page.screenshot(path="/tmp/sigaa_consolidar_discente_fail.png")
                print("   [DEBUG] Screenshot salvo em /tmp/sigaa_consolidar_discente_fail.png")
            except Exception:
                pass
            raise RuntimeError(
        ...
        # Aguardar pagina de detalhes da consolidacao carregar
        try:
            await page.wait_for_load_state("domcontentloaded", timeout=10000)
        except Exception:
            pass
        ...
        print(f"   [DEBUG] URL apos selecao: {page.url}")

        # ── SELECIONAR CONCEITO ────────────────────────────────────────────
        print(f"[7/8] Selecionando Conceito '{entrada.conceito}'...")

        # Debug: listar selects e inputs na pagina
        try:
            page_debug = await page.evaluate("""() => {
                const info = {selects: [], submits: [], passwords: []};
                document.querySelectorAll('select').forEach(s => {
                    const opts = [...s.options].map(o => o.text.trim() + '=' + o.value);
                    info.selects.push({id: s.id, name: s.name, options: opts.slice(0, 15)});
                });
                document.querySelectorAll('input[type=submit], input[type=button], button').forEach(b => {
                    info.submits.push({tag: b.tagName, id: b.id, name: b.name, value: b.value, text: b.textContent?.trim()});
                });
                document.querySelectorAll('input[type=password]').forEach(p => {
                    info.passwords.push({id: p.id, name: p.name});
                });
                return info;
            }""")
            print(f"   [DEBUG] Pagina consolidacao: {json.dumps(page_debug, indent=2, ensure_ascii=False)}")
        except Exception as e:
            print(f"   [DEBUG] Falha ao debugar pagina: {e}")

        # Tentar selecionar o conceito no dropdown
        conceito_selecionado = False

        # 1. Tentar por id/name contendo "conceito" ou "resultado"
        for sel_id in ["select[id*='conceito' i]", "select[id*='resultado' i]",
            ...
            try:
                ...
                if await sel.count():
                    # Tentar selecionar pela label
                    try:
                        await sel.select_option(label=entrada.conceito, timeout=3000)
                        conceito_selecionado = True
                        print(f"   [OK] Conceito '{entrada.conceito}' selecionado via {sel_id}")
                        break
                    except Exception:
                        pass
                    # Tentar pelo value
                    try:
                        await sel.select_option(value=entrada.conceito, timeout=3000)
                        conceito_selecionado = True
                        print(f"   [OK] Conceito '{entrada.conceito}' selecionado (value) via {sel_id}")
                        break
                    except Exception:
                        pass
            except Exception:
        ...
        # 2. Fallback: qualquer select que tenha a opção "E" entre suas opções
        if not conceito_selecionado:
            conceito_selecionado = await selecionar_opcao_em_dropdown(
                page, entrada.conceito, filtro_dropdown=None
            )
            if conceito_selecionado:
                print(f"   [OK] Conceito '{entrada.conceito}' selecionado via dropdown generico.")

        if not conceito_selecionado:
            print(f"   [WARN] Nao foi possivel selecionar conceito '{entrada.conceito}' automaticamente.")
            print("   Tentando continuar (pode ja estar pre-selecionado)...")

        # ── PROXIMO PASSO (botão entre conceito e senha) ───────────────────
        print("[8/9] Clicando 'Proximo Passo'...")
        ...
        if not clicou_prox:
            # Tentar via JS direto no botão form:btnConfirmacao
            try:
                await page.evaluate("""() => {
        ...
        # Aguardar proxima pagina carregar
        try:
            await page.wait_for_load_state("domcontentloaded", timeout=10000)
        except Exception:
            pass
        ...
        # Debug: ver o que tem na pagina agora
        try:
            page_debug2 = await page.evaluate("""() => {
                const info = {selects: [], submits: [], passwords: [], inputs: []};
                document.querySelectorAll('select').forEach(s => {
                    const opts = [...s.options].map(o => o.text.trim() + '=' + o.value);
                    info.selects.push({id: s.id, name: s.name, options: opts.slice(0, 10)});
                });
                document.querySelectorAll('input[type=submit], input[type=button], button').forEach(b => {
                    info.submits.push({tag: b.tagName, id: b.id, name: b.name, value: b.value, text: b.textContent?.trim()});
                });
                ...
                document.querySelectorAll('input[type=text], input[type=hidden]').forEach(i => {
                    if (i.id && (i.id.includes('senha') || i.id.includes('Senha') || i.id.includes('password')))
                        info.inputs.push({id: i.id, name: i.name, type: i.type});
                });
                return info;
            }""")
            print(f"   [DEBUG] Pagina apos Proximo Passo: {json.dumps(page_debug2, indent=2, ensure_ascii=False)}")
        except Exception as e:
            print(f"   [DEBUG] Falha ao debugar: {e}")
        ...
        try:
            ...
            if not await preencher_primeiro_visivel(
                page,
                ["input[id*='senha']", "input[id*='Senha']",
                 "input[type='password']", "input[name*='senha']"],
                cfg.senha,
            ):
                # Pode ser que esta pagina nao precise de senha
                body_text = (await page.locator("body").inner_text()).lower()
                if "senha" in body_text:
                    raise RuntimeError("Nao foi possivel preencher a senha de confirmacao.")
                else:
                    print("   [INFO] Pagina nao tem campo de senha. Continuando...")
        ...
        if args.executar:
            ...
            if not clicou:
                raise RuntimeError("Nao foi possivel clicar em Confirmar/Consolidar.")
            ...
            if "sucesso" in conteudo or "consolidada" in conteudo or "realizada" in conteudo:
                print("[OK] Consolidacao executada — mensagem de sucesso detectada.")
            else:
            ...
            print("[DRY-RUN] Parado antes de Confirmar. Senha preenchida mas NAO enviada.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Consolida matricula em atividade complementar/TCC no SIGAA"
    )
    ...
    parser.add_argument("--componente", required=True, help="ACC I, ACC II, ACC III, ACC IV, TCC I, TCC II")
    parser.add_argument("--conceito", default="E", help="Conceito a atribuir (padrao: E)")
