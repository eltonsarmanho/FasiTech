...
\begin{center}
  ...
  {\small Data de implementação: 01/10/2025 \quad|\quad
   Unidade: Faculdade de Sistemas de Informação -- Campus Tocantins-Cametá / UFPA}
\end{center}
...
A Faculdade de Sistemas de Informação (FASI) da UFPA, Campus Tocantins-Cametá, atende
estudantes distribuídos em três polos geograficamente dispersos: Cametá, Limoeiro do Ajuru
e Oeiras do Pará. Essa configuração \textit{multicampi} impunha desafios que o modelo de
...
\begin{itemize}[label={\color{ufpared}\faExclamationCircle}]
  ...
  \item \textbf{Deslocamento obrigatório em contexto de alta vulnerabilidade:} Levantamento
    ...
    os polos mais distantes da sede (Limoeiro do Ajuru e Oeiras do Pará) concentram 75,7\%
...
desenvolvida para a FASI/UFPA. Reúne em um único ambiente web os principais processos de
...
\node[mstud] (acc)  at (-3.6, 2.2)  {\faFileAlt\; ACC};
...
\node[mstud] (ale)  at ( 3.6, 0)    {\faBell\; Alertas\\[-1pt]Acadêmicos};
...
\begin{itemize}[label={\color{accentor}\faRobot}]
  \item \textbf{Módulo ACC com IA:} Os certificados são enviados como arquivos PDF.
    ...
    documento, elimina o cálculo manual e retorna ao estudante um resumo validado --
    processo que antes consumia horas da secretaria.
...
(Cametá, Limoeiro do Ajuru, Oeiras do Pará), o corpo docente e a equipe técnico-administrativa
da FASI/UFPA Campus Tocantins-Cametá.
...
que se beneficia de indicadores estruturados de uma unidade multicampi no interior.
...
\begin{center}
\begin{tikzpicture}[
  ph/.style={rectangle, rounded corners=4pt, text width=2.6cm, minimum height=1.7cm,
...
Primeiro mês: \textbf{66 submissões} -- evidência imediata de adoção espontânea.
...
registrou \textbf{173 submissões} -- pico absoluto, coincidente com período de prazos
...
Consolidação de \textbf{452 registros em 10 tabelas transacionais}.
...
\begin{enumerate}[label={\color{darkgreen}\textbf{\arabic*.}}, itemsep=5pt]
  \item \textbf{Primeira plataforma integrada da unidade:} Antes do FasiTech, não existia
    qualquer sistema digital unificado para tramitação acadêmica na FASI. A inovação não
  ...
  \item \textbf{IA aplicada à burocracia acadêmica:} O uso de Inteligência Artificial para
...
\midrule
Trâmites acadêmicos 100\% digitalizados      & \textbf{218 processos} & \textcolor{darkgreen}{\faCheckCircle} \\
...
Polos mais distantes (Limoeiro + Oeiras)     & \textbf{75,7\% dos envios} & \textcolor{darkgreen}{\faCheckCircle} \\
...
Satisfação com transparência da gestão       & \textbf{4,43\,/\,5,0} & \textcolor{accentor}{\faExclamationCircle}~baseline \\
...
\subsection*{7.2\;Impacto na equidade -- interiorização real}
...
TCC e Estágio} -- exatamente o grupo que mais sofria com o modelo presencial/papel
anterior. O FasiTech atende mais intensamente quem mais precisava de um canal remoto.

\begin{center}
\begin{tikzpicture}
\foreach \polo/\val/\col/\y in {
  {Limoeiro do Ajuru}/40.3/ufpablue!80/1.5,
  {Oeiras do Pará}/35.4/ufpablue!55/0.75,
  {Cametá}/22.9/ufpablue!28/0.0
}{
...
Avaliação de gestão (n=9, março/2026): \textbf{transparência = 4,43/5} (ponto mais forte),
suporte = 4,17/5, comunicação = 4,00/5. As respostas abertas revelam percepção positiva
do canal digital e demandas por maior equidade no acesso a oportunidades -- insumo
direto para planejamento da coordenação.
...
\textbf{Infraestrutura} &
  Laboratórios de informática e conectividade já disponíveis na FASI &
...
\begin{tcolorbox}[warnbox]
  ...
  neste módulo -- sem contabilizar os ganhos dos outros 9 módulos digitalizados.
\end{tcolorbox}
...
\begin{itemize}[label={\color{ufpablue}\faLock}, itemsep=4pt]
  \item \textbf{Avaliação de Gestão com publicidade interna:} Estudantes e docentes
    ...
    são consolidados e apresentados à coordenação, criando ciclo institucional de
    melhoria contínua. Resultado atual: \textbf{transparência = 4,43/5 -- dimensão de
    maior pontuação na avaliação}.

  \item \textbf{Alertas acadêmicos com histórico auditável:} Cada notificação é
    armazenada com data de criação, data de disparo e público destinatário -- histórico
  ...
  \item \textbf{Acesso igualitário entre polos:} A plataforma elimina a assimetria
    informacional do modelo presencial: estudantes de Limoeiro do Ajuru e Oeiras do Pará
...
Exclusão digital (14,2\% sem internet estável; 25\% sem computador próprio) &
  ...
  Interface responsiva para smartphone; orientação para uso dos laboratórios da FASI \\
\addlinespace
...
\begin{itemize}[label={\color{darkgreen}\faStar}, itemsep=5pt]
  ...
  \item \textbf{Instrumentalização da própria plataforma para gerar evidências:} A
    ...
    início permitiu extrair indicadores concretos de impacto -- retroalimentando o
...
\begin{tcolorbox}[metricbox]
  ...
  \textbf{vontade institucional} (legitimidade junto à coordenação e à comunidade),
...
\begin{tcolorbox}[successbox]
  ...
  qualquer unidade multicampi da UFPA ative apenas os módulos pertinentes ao seu
  ...
  A experiência da FASI/UFPA Campus Tocantins-Cametá pode ser replicada em qualquer
