...
\begin{center}
  \includegraphics[width=0.49\linewidth]{DashboardGestao_View1}\hfill%
  \includegraphics[width=0.49\linewidth]{DashboardGestaio_View2}\\[2pt]
  {\scriptsize\textit{Dashboard de gestão: avaliação por dimensão (esq.) e impacto percebido do FasiTech -- 82\% positivo (dir.).}}
\end{center}
...
\begin{itemize}[label={\color{ufpablue}\faLock}, itemsep=2pt]
...
\begin{tabular}{L{4.6cm} C{1.6cm} L{5.8cm}}
\toprule
\textbf{Barreira} & \textbf{Impacto} & \textbf{Como foi superada} \\
\midrule
Exclusão digital (14,2\% sem internet estável; 25\% sem computador) &
  \textcolor{ufpared}{\textbf{Alto}} &
  Interface responsiva para smartphone; acesso via laboratórios da unidade \\
Integração com SIGAA para automação plena de notas &
  \textcolor{ufpared}{\textbf{Alto}} &
  Módulo autônomo de lançamento de conceitos (parcialmente funcional); solução completa depende de microserviços do CTIC para acesso direto sem interface gráfica \\
Adaptação de docentes e secretaria ao novo fluxo &
  \textcolor{accentor}{\textbf{Médio}} &
  Benefícios imediatos visíveis (menos trabalho manual); treinamento contextualizado \\
Custo operacional de modelos de IA &
  \textcolor{accentor}{\textbf{Médio}} &
  Prompts otimizados com Gemini e MariTalk; próxima etapa: modelo open-source para escalabilidade \\
Zero orçamento adicional &
  \textcolor{accentor}{\textbf{Médio}} &
  Stack 100\% open-source; hospedagem de baixo custo \\
Qualidade dos dados históricos &
  \textcolor{accentor}{\textbf{Médio}} &
  Correção incremental; campos obrigatórios em todos os novos formulários \\
Desenvolvimento paralelo às obrigações docentes &
  \textcolor{accentor}{\textbf{Médio}} &
  Ciclos curtos; priorização por impacto imediato no usuário \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[label={\color{darkgreen}\faStar}, itemsep=2pt]
...
% ============================================================
O item em questão não se aplica à ação inovadora em processos organizacionais
